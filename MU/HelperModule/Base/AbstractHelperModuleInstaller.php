...
{
    /**
     * Install the MUHelperModule application.
     *
    ...
    public function install()
    {
        ...
        try {
            ...
            $logger->error('{app}: Could not create the database tables during installation. Error details: {errorMessage}.', ['app' => 'MUHelperModule', 'errorMessage' => $exception->getMessage()]);
    
            return false;
        }
    
        // set up all our vars with initial values
        $this->setVar('accessKey', '');
        $this->setVar('secretKey', '');
        $this->setVar('associateKey', '');
        $this->setVar('passwordForImport', '');
        $this->setVar('amountForImport', '4');
        $this->setVar('textForDetailLink', 'Bei Amazon erh?ltlich');
    ...
    /**
     * Upgrade the MUHelperModule application from an older version.
     *
        ...
        switch ($oldVersion) {
            case '1.0.0':
                ...
                try {
                    ...
                    $this->addFlash('error', $this->__('Doctrine Exception') . ': ' . $exception->getMessage());
                    $logger->error('{app}: Could not update the database tables during the upgrade. Error details: {errorMessage}.', ['app' => 'MUHelperModule', 'errorMessage' => $exception->getMessage()]);
    ...
    */
    ...
    public function uninstall()
    {
        ...
        try {
            ...
            $this->addFlash('error', $this->__('Doctrine Exception') . ': ' . $exception->getMessage());
            $logger->error('{app}: Could not remove the database tables during uninstallation. Error details: {errorMessage}.', ['app' => 'MUHelperModule', 'errorMessage' => $exception->getMessage()]);
    ...
    /**
     * Build array with all entity classes for MUHelperModule.
     *
    ...
    {
        $classNames = [];
        $classNames[] = 'MU\HelperModule\Entity\ProductClassEntity';
