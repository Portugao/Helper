...
namespace MU\HelperModule\Form\Handler\ProductClass\Base;

use MU\HelperModule\Form\Handler\Common\EditHandler;
use MU\HelperModule\Form\Type\ProductClassType;
 ...
 * It aims on the product class object type.
 */
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'productClass';
        $this->objectTypeCapital = 'ProductClass';
        $this->objectTypeLower = 'productclass';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the product class yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'MUHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(ProductClassType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of product class area
        $codes[] = 'userIndex';
        // admin index page of product class area
        $codes[] = 'adminIndex';
        // user list of product classes
        $codes[] = 'userView';
        // admin list of product classes
        $codes[] = 'adminView';
        // user detail page of treated product class
        $codes[] = 'userDisplay';
        // admin detail page of treated product class
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        $routePrefix = 'muhelpermodule_' . $this->objectTypeLower . '_' . $routeArea;
    
        // redirect to the list of productClasses
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Product class created.');
                } else {
                    $message = $this->__('Done! Product class updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Product class deleted.');
                break;
            default:
                $message = $this->__('Done! Product class updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'product class', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        if ($this->request->getSession()->has('muhelpermodule' . $this->objectTypeCapital . 'Referer')) {
            $this->request->getSession()->remove('muhelpermodule' . $this->objectTypeCapital . 'Referer');
        }
